...
تم تنظيم المشروع داخل هيكل معماري حديث يعتمد على **TypeScript** وقاعدة بيانات **PostgreSQL + pgvector** لضمان الأمان النوعي، السرعة العالية، والفصل التام بين الطبقات:

```text
EyraOS/
├── src/                             # الكود المصدري بلغة TypeScript
│   ├── types/                       # تعريفات الأنواع والواجهات (MemoryStatus, Entity, Relation, Procedure)
│   │   └── memory.types.ts
│   ├── config/                      # ضبط البيئة والاتصال
│   │   └── env.ts
│   ├── db/                          # طبقة قاعدة البيانات
│   │   ├── connection.ts            # مجمع الاتصالات (PostgreSQL Pool)
│   │   └── repositories/            # مستودعات الاستعلام (Entity, Relation, Episode, Procedure)
│   ├── core/                        # محركات الإدراك والمعالجة الأساسية
│   │   ├── arabic.ts                # معالجة النصوص العربية وتجريد السوابق والتوحيد
│   │   ├── conflict.ts              # محرك فض التناقضات وحالات القناعات (FSM)
│   │   ├── retrieval.ts             # الاسترجاع الانتقائي متدرج الطبقات
│   │   ├── memory.engine.ts         # محرك الذاكرة المنسق
│   │   └── session.manager.ts       # مدير الجلسات والذاكرة العاملة المؤقتة
│   ├── services/                    # الخدمات الخارجية والتأمل المعرفي
│   │   ├── gemini.service.ts        # معالجة حوارات الذكاء الاصطناعي واستخراج الحقائق
│   │   └── reflection.service.ts    # دورات التأمل المعرفي وتوحيد الاستنتاجات
│   ├── routes/                      # موجهات واجهات الـ REST API
│   │   ├── memory.routes.ts         # مسارات الرسم البياني والإحصائيات والإجراءات
│   │   └── chat.routes.ts           # مسار الحوار وإدارة الجلسات
│   ├── app.ts                       # إعداد تطبيق Express والبرمجيات الوسيطة
│   └── server.ts                    # نقطة الانطلاق الرئيسية وفحص الاتصال
├── tests/                           # حزمة الاختبارات الآلية الشاملة (Jest)
│   ├── arabic.test.ts               # اختبارات معالجة وتطبيع اللغة العربية
│   └── conflict.test.ts             # اختبارات فض التناقضات والعلاقات الحصرية
...
├── .env                             # متغيرات البيئة المحلية وقاعدة البيانات
├── .env.example                     # نموذج متغيرات البيئة المرجعي
├── tsconfig.json                    # إعدادات مترجم TypeScript
├── jest.config.js                   # إعدادات محرك الاختبارات الآلية
